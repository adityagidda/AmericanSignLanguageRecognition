...
#def to_tf_format(imgs):
 #   return np.stack([img[:, :] for img in imgs]).astype(np.float32)
  ...
  for each in big: 
    for c,s in enumerate(small):
     ...
     for img_name in glob.glob(full_path + "color*.png"):
      ...
      images.append(reimg.astype(np.float32))
      labels.append(label)

  return Dataset(X = np.array(images).astype(np.float32),
...
#Here comes the tensorflow part with the CNN
